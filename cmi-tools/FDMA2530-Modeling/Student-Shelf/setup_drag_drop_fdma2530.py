...
"""
=======
CMI Tools ΓÇô FDMA 2530 Shelf Installer
=====================================================

Drag-and-drop script that installs or loads the FDMA 2530 student
shelf.  Key features:

ΓÇó Downloads the repository as a ZIP (fast, no git required).  
ΓÇó Copies `fdma_shelf`, `shelf_config.json`, **and `releases.json`**  
  so version_utils can find the manifest.  
ΓÇó Reads `current_version` from the manifest ΓÇô no hard-coded numbers.  
ΓÇó Creates a Maya `.mod` file so the shelf auto-loads.  
ΓÇó Works on Windows / macOS / Linux, Maya 2016-2025, Py 2/3.  

Author:  Alexander T. Santiago   ΓÇó   asanti89@nmsu.edu
"""
...
def safe_download(url, timeout=30):
    ...
    try:
        LOG.info("Downloading %s ΓÇª", url)
...
def _find_repo_root(extract_dir):
    """Return the ΓÇª/NMSU_Scripts-xxxx root inside *extract_dir*."""
...
=======
        ...
        LOG.info("Copied fdma_shelf ΓåÆ %s", dst_pkg)
...
def show_install_dialog():
    ...
    choice = cmds.confirmDialog(
        ...
        message=(
            ...
            "ΓÇó Install ΓÇô permanent (Maya modules)\n"
            "ΓÇó Load Once ΓÇô session only\n"
            "ΓÇó Uninstall ΓÇô remove everything"
        ),
>>>>>>> bd60b003fa2b3ac6c11a49f980642cfa6d6f37af
    ...
    if choice == "Install":
        if install_permanent():
            ...
            cmds.confirmDialog(
                title="Error",
                message="Installation failed ΓÇô check Script Editor.",
                button=["OK"],
            )
<<<<<<< HEAD
...
>>>>>>> bd60b003fa2b3ac6c11a49f980642cfa6d6f37af
            ...
            cmds.confirmDialog(
                title="Error",
                message="Temporary load failed ΓÇô check Script Editor.",
                button=["OK"],
            )
<<<<<<< HEAD
